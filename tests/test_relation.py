...
class TestRelation:
    def test_add_creates_relation_and_inverse(self, runner):
        ...
        assert any(r["kind"] == "subtask" and r["other_task_id"] == 3 for r in source["relations"])
    ...
    def test_remove_removes_relation_and_inverse(self, runner):
        ...
        target = json_for_task_id(runner, 3)
        assert not any(r["kind"] == "parenttask" and r["other_task_id"] == 1 for r in target["relations"])
