...
class Urgency:
    ...
    def _get_label_score(self, task):
        ...
        return (
            int(any(label_name.lower() in task_label_title for label_name in self._label_keywords))
            if self._label_keywords
            else 0
        )
    ...
    def _get_project_score(self, task):
        ...
        return (
            int(any(project_name.lower() in task_project_title for project_name in self._project_keywords))
            if self._project_keywords
            else 0
        )
